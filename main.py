        ...
        if len(cmds) > 3:
            ...
            if len(self.initiativeQueue) == charKeysLen:
                ...
                for k,v in initQSorted.items():
                    initQSorted[k] = [chars[k], initQSorted[k]]
    ...
    def table_show(self):
        if not self.combatReady:
            ...
            for k, v in self.initiativeQueue.items():
                ...
                i += 1
            print("==================================")
            print("It is ", charKeys[self.turn], "'s turn")
            print(self.initiativeQueue[charKeys[self.turn]][0].describe())
    
    def turn_pass(self):
        if self.turn == charKeysLen -1 :
                self.turn = 0
        else:
            charKeys = list(self.initiativeQueue.keys())
            if self.initiativeQueue[charKeys[self.turn]][0].hp < 0:
                self.turn_pass()
            self.turn += 1
        self.table_show()
